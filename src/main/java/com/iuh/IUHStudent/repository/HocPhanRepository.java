...
public interface HocPhanRepository extends JpaRepository<HocPhan,Integer> {
    @Query("select hp.hocPhanId,hp.maHocPhan,hp.moTa,hp.batBuoc from HocPhan hp  join MonHoc mh on hp.monHoc.monHocId = mh.monHocId where mh.khoaVien.khoaVienId = ?1")
